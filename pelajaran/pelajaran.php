							...
							<div class="d-flex justify-content-between">
								...
								?>
									<div class="input-group mb-3">
										<input type="text" class="form-control" placeholder="Cari Mata Pelajaran" name="cari"
										value="<?= $cari ?>">
										<button class="btn btn-secondary" type="submit" id="btnCari">
								...
								$page = 5;
									...
									$no = $start + 1;
									$queryPelajaran = mysqli_query($koneksi, "SELECT * FROM tbl_pelajaran WHERE
									pelajaran like '%$keyword%' or jurusan like '%$keyword%' or guru like '%$keyword%'
									limit $page offset $start");
									
									$queryJmlData = mysqli_query($koneksi, "SELECT * FROM tbl_pelajaran WHERE
									pelajaran like '%$keyword%' or jurusan like '%$keyword%' or guru like '%$keyword%' ");
								...
								<tbody>
									...
									<tr>
										...
										<td align="center">
											...
											class="btn btn-sm btn-warning" title="update pelajaran">
											<i class="fa-solid fa-pen"></i></a>
											
											<button type="button" data-id="<?= $data['id'] ?>"
											id="btnHapus" class="btn btn-sm btn-danger" title="hapus pelajaran">
								...
								<?php }
									for($hal = 1; $hal <= $pages; $hal++){
										if($hal == $halaktif){ ?>
											...
											href="?hal=<?= $hal ?>&cari=<?= $keyword ?>"><?= $hal ?></a></li>
											<?php } else { ?>
